...
class simulation(object):
    ...
    def simulate(self, _savePath):
        visualize = visualization(_savePath)
...
class visualization(object):
    ...
    def plotRoad(self,roadId,img):
        ...
        cv.putText(img,str(roadId),(int((fromX+toX)/2),int((fromY+toY)/2)),cv.FONT_HERSHEY_SIMPLEX,0.6,[255,0,0],2)
        cv.putText(img,str(road.__speed__()),(int((fromX+toX)/2),int((fromY+toY)/2+20)),cv.FONT_HERSHEY_PLAIN,1,[0,0,255],2)
        cv.putText(img,str(road.__length__()),(int((fromX+toX)/2),int((fromY+toY)/2+40)),cv.FONT_HERSHEY_PLAIN,1,[94,160,24],2)
        # plot bucket
        ...
        for i in range(length):
            for j in range(channel):
                ...
                if bucket[i][j] is  None:
                    cv.rectangle(img,(int(XY[0]),int(XY[1])),(xRD,yRD),(112,112,112),1)
                else:
    ...
    def plotInfo(self,img):
        ...
        cv.putText(img, "in the carport:%d,on the road:%d,end of the trip:%d" % (CARDISTRIBUTION[0],CARDISTRIBUTION[1],CARDISTRIBUTION[2]),(30,30), cv.FONT_HERSHEY_SIMPLEX, 0.6, [0, 0, 255], 2)
        cv.putText(img,"roadId",(30,200),cv.FONT_HERSHEY_SIMPLEX,0.6,[255,0,0],2)
        cv.putText(img,"maxSpeed",(30,220),cv.FONT_HERSHEY_PLAIN,1,[0,0,255],2)
        cv.putText(img, "length", (30, 240),cv.FONT_HERSHEY_PLAIN, 1, [94, 160, 24], 2)
...
def main():
    ...
    for line in carInfo:
        id_, from_, to_, speed_, planTime_, priority, preset = line.replace(' ', '').replace('\t', '')[1:-1].split(',')
