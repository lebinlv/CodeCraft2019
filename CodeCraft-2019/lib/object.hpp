...
class Container
{
  ...
  public:
     ...
     */
    void search_road(CAR* car);

    /**
     * @brief 得到这条路上车的总数，也就是拥塞程度的一个指标
     */
    int get_car_sum();
};
...
struct CROSS
{
    ...
    * 第二个是全局时间
    */
    void drive_car_init_list(bool is_prior,int global_time);

    Container* searchRoadForCar(Container *current_container);
};
